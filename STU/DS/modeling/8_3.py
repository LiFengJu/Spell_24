...
R = {}
for i in range(1, 6):
    R[i] = model.addVar(name=f"R{i}")

# 定义R均值变量
Ravg = model.addVar(name="Ravg")
...
model.addConstr(Ravg == (R[1] + R[2] + R[3] + R[4] + R[5]) / 5)
model.addConstr(Ravg >= 10)

# 定义目标函数
model.setObjective((1/5) * gp.quicksum((R[i] - Ravg) ** 2 for i in range(1, 6)), sense=gp.GRB.MINIMIZE)

...
--- run 运行状态机
function conveyer1:run(conf)
    local client = {
        id = conf.id or "conveyer1",
        ip = conf.ip or "127.0.0.1"
    }
    setmetatable(client, { __index = self })
    client:subscribe()
    timer.afterFunc(3000, function()
        client:changeState("s_conveyer1_connect")
    end)
    return client
end

--- changeState 改变状态机当前状态
function conveyer1:changeState(name, ...)
    log(name)
...
--- publish 发布消息
function conveyer1:publish(data)
    mqtt:Publish(self.id, data)
end

--- connected 已连接运动控制器
function conveyer1:connected()
    return self.handle ~= nil
end

--- connected 已连接运动控制器
function conveyer1:createTimer(key, interval, callback)
...
---   key 定时器名称(如果为空则删除全部)
function conveyer1:removeTimer(key)
    if key==nil then
        for _, timerId in pairs(self.stateTimer) do
            timer.removeTimer(timerId)
        end

...
class Server:
    ...
    def serve(self, user):
        if self.service_flag and user.served[self.level] == 0 and user.bandwidth_request <= self.bandwidth.level:
            ...
            user.serve_over = user.env.event()
            # 记录用户的服务情况
...
df = pd.DataFrame(np.arange(11,211).reshape((40,5)).T,index = ['A','B','C','D','E'])


def change(W,servers):
    for i in df:
        index = df.at[W,i]
        servers[index].change_bandwidth_capacity()
def contral_bandwidth(self,servers):
    while True:
        yield self.env.timeout(570)#9:30
        change('A',servers)
        
        yield self.env.timeout(70)#10:40
        change('B',servers)#会持续到11：50
        yield self.env.timeout(490)#19：00
        change('C',servers)
        yield self.env.timeout(70)#20：10
        change('D',servers)
        yield self.env.timeout(70)#21：20
        change('E',servers)
        yield self.env.timeout(160)#00：00
